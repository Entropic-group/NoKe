...
def normalize_and_clean_party_affiliations(datapoints: List[Dict]) -> List[Dict]:
    ...
    for datapoint in datapoints:
        ...
        party = normalized_datapoint.get("party_affiliation")
        if not party or party.strip().lower() not in PARTY_AFFILIATIONS:
            normalized_datapoint["party_affiliation"] = "none"
        else:
            normalized_datapoint["party_affiliation"] = party.strip().lower()
    ...
    for idx, datapoint in enumerate(datapoints):
        ...
        for count_col in ["barely_true_count",
            ...
            if count_col in normalized_datapoint:
                try:
                    if normalized_datapoint[count_col] is None or normalized_datapoint[count_col] == "":
                        normalized_datapoint[count_col] = float(0.0)  # or 0.0 if you prefer
                    else:
